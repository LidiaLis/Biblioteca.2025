 ...
 * @author Lídia Lis
 */
public class DialogAcervo extends javax.swing.JDialog {
   ...
   private Livro livroSelecionado = null;
     ...
     * @param idLivro
     */
    public DialogAcervo(java.awt.Frame parent, boolean modal, Integer idLivro) {
        super(parent, modal);
        initComponents();
        setLocationRelativeTo(parent);
        if (idLivro != null) {
        LivroDAO dao = new LivroDAOJDBC(); // ou como você instanciar seu DAO
        livroSelecionado = dao.buscaPorId(idLivro); // busca o livro completo pelo ID
    ...
    if (livroSelecionado != null) {
        ...
        ftxtData.setText(sdf.format(livroSelecionado.getData_doacao()));

        checkBDisponivel.setSelected(livroSelecionado.getDisponivel());
        // Seleciona o doador na combo
        Usuario doador = livroSelecionado.getId_doador();
        cbUsuario.setSelectedItem(doador);
        }
        }
    }   
    ...
    try {
           ...
           if (livroSelecionado == null) {
            ...
            novoLivro.setData_doacao(utilDate);
            novoLivro.setDisponivel(disponivel);
            novoLivro.setId_doador(usuarioSelecionado);

            int sucesso = dao.inserir(novoLivro);
            if (sucesso > 0) {
            ...
            livroSelecionado.setData_doacao(utilDate);
            livroSelecionado.setDisponivel(disponivel);
            livroSelecionado.setId_doador(usuarioSelecionado);

            int sucesso = dao.editar(livroSelecionado);
    ...
    // Atualiza a combo de usuários (doador)
    ...
    // Se estiver em modo edição, seleciona o doador correto
    if (livroSelecionado != null) {
        for (int i = 0; i < modeloUsuarios.getSize(); i++) {
            Usuario u = modeloUsuarios.getElementAt(i);
            if (u.getId_usuario() == livroSelecionado.getId_doador().getId_usuario()) {
                cbUsuario.setSelectedIndex(i);
                break;
            }
        }
    } else {
        // Se for inserção, seleciona o primeiro usuário por padrão
